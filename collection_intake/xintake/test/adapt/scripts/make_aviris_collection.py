...
def get_time( fname: str ):
    ...
    return parse( fname[time_start:time_end] )
...
for agg_name, agg_files_glob in aggregations.items():
    ...
    for data_file in agg_files:
        ...
        entry = AvirisFileCatEntry( str(time), agg_name, data_file )
    ...
    sub_collections.append( collection.generate( cats = cat_files ) )

root_collection = Collection( description="ORNL ABoVE Airborne AVIRIS NG" )
root_collection.generate( cats=sub_collections )
