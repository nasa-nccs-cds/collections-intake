...
class AvirisFileCatEntry(Aggregation):
    ...
    def __init__(self, fileId: str, collection: str, filePath: str,  **kwargs):
        super(AvirisFileCatEntry, self).__init__( name=fileId, collection=collection, files=[filePath], **kwargs )

    def getDataSource(self, **kwargs ) -> DataSource:
        try:
            intake.open_rasterio(self.files[0])
